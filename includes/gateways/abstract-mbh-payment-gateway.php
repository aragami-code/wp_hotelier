...
abstract class MBH_Payment_Gateway {
	/**
	 ...
	 *
	 * @param MBH_Reservation $reservation
	 * @return string
	 */
	public function get_return_url( $reservation = null ) {
		...
		if ( $reservation ) {
			...
			$return_url = mbh_get_endpoint_url( 'reservation-received', '', mbh_get_page_permalink( 'booking' ) );
		}
		...
		return apply_filters( 'mb_hms_get_return_url', $return_url, $reservation );
	}

	/**
	 * Get a link to the transaction on the 3rd party gateway size (if applicable)
	 *
	 * @param  mbh_Reservation $reservation the reservation object
	...
	public function get_transaction_url( $reservation ) {
		...
		return apply_filters( 'mb_hms_get_transaction_url', $return_url, $reservation, $this );
	}
	...
	protected function get_reservation_deposit() {
		...
		} elseif ( 0 < MBH()->cart->get_required_deposit() ) {
			$total = MBH()->cart->get_required_deposit();
		}
	...
	public function get_title() {
		return apply_filters( 'mb_hms_gateway_title', $this->title, $this->id );
	}
	...
	public function get_description() {
		return apply_filters( 'mb_hms_gateway_description', $this->description, $this->id );
	}
	...
	public function get_icon() {
		$icon = $this->icon ? '<img src="' . MBH_HTTPS::force_https_url( $this->icon ) . '" alt="' . esc_attr( $this->get_title() ) . '" />' : '';

		return apply_filters( 'mb_hms_gateway_icon', $icon, $this->id );
	}
	...
	public function is_available() {
		$enabled = mbh_get_option( 'payment_gateways', false );
		$is_available = false;
	...
	public function supports( $feature ) {
		return apply_filters( 'mb_hms_payment_gateway_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
