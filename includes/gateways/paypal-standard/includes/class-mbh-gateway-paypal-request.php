...
if ( ! class_exists( 'MBH_Gateway_Paypal_Request' ) ) :

/**
 * MBH_Gateway_Paypal_Request Class
 */
class MBH_Gateway_Paypal_Request {
	...
	public function __construct( $gateway ) {
		...
		$this->notify_url = MBH()->api_request_url( 'MBH_Gateway_Paypal' );
	}
	...
	protected function get_paypal_args( $reservation ) {
		MBH_Gateway_Paypal::log( 'Generating payment for reservation ' . $reservation->get_reservation_number() . '. Notify URL: ' . $this->notify_url );

		return apply_filters( 'mb_hms_paypal_args', array_merge(
			array(
				'cmd'           => '_cart',
				'business'      => $this->gateway->email,
				'no_note'       => 1,
				'currency_code' => mbh_get_currency(),
				'charset'       => 'utf-8',
				'rm'            => is_ssl() ? 2 : 1,
				'upload'        => 1,
				...
				'page_style'    => mbh_get_option( 'paypal_page_style' ),
				'bn'            => 'mb_hms_Cart',
	...
	protected function get_line_item_args( $reservation ) {
		 ...
		 */
			...
			$this->add_line_item( $all_items_name ? $all_items_name : esc_html__( 'Reservation', 'wp-mb_hms' ), 1, $reservation->get_deposit(), $reservation->get_reservation_currency() );
			$line_item_args = $this->get_line_items();
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0, $currency ) {
		...
		$amount = $this->number_format( mbh_convert_to_cents( $amount ), $currency );

		$this->line_items[ 'item_name_' . $index ]   = html_entity_decode( MBH_Formatting_Helper::trim_string( $item_name, 127 ), ENT_NOQUOTES, 'UTF-8' );
	...
	protected function get_reservation_item_name( $item ) {
		...
		if ( $rate ) {
			$item_name .= ' ( ' . 	mbh_get_formatted_room_rate( $rate ) . ' )';
		}
