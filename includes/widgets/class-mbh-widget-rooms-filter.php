...
class MBH_Widget_Rooms_Filter extends MBH_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'widget--mb_hms widget-rooms-filter';
		$this->widget_description = __( 'Displays a custom filter which lets you narrow down the list of rooms. Visible only in the "listing" page.', 'wp-mb_hms' );
		$this->widget_id          = 'mb_hms-widget-rooms-filter';
		$this->widget_name        = __( 'mb_hms Rooms Filter', 'wp-mb_hms' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Refine results', 'wp-mb_hms' ),
				'label' => __( 'Title', 'wp-mb_hms' )
			),
			'max_guests' => array(
				'type'  => 'number',
				'std'   => 7,
				'label' => __( 'Max guests (type "0" to hide this filter)', 'wp-mb_hms' )
			),
			'max_children' => array(
				'type'  => 'number',
				'std'   => 3,
				'label' => __( 'Max children (type "0" to hide this filter)', 'wp-mb_hms' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		do_action( 'mb_hms_before_widget_rooms_filter' );
		?>

		<div class="widget-rooms-filter__wrapper">

			<?php
			$link            = MBH()->cart->get_room_list_form_url();
			$room_cats       = get_terms( 'room_cat' );
			$room_rates      = get_terms( 'room_rate' );
			$default_filters = apply_filters( 'mb_hms_widget_rooms_filter_default_filters',
				array(
					'room_cat',
					'room_rate',
					'guests',
			...
			do_action( 'mb_hms_before_widget_rooms_filter', $default_filters );
			...
			if ( ! empty( $room_cats ) && ! is_wp_error( $room_cats ) ) :
				...
				<div class="widget-rooms-filter__group widget-rooms-filter__group--room-type">
					...
					<span class="widget-rooms-filter__group-label"><?php esc_html_e( 'Room type', 'wp-mb_hms' ); ?></span>
			...
			if ( ! empty( $room_rates ) && ! is_wp_error( $room_rates ) ) :
				...
				<div class="widget-rooms-filter__group widget-rooms-filter__group--room-rate">
					...
					<span class="widget-rooms-filter__group-label"><?php esc_html_e( 'Room rate', 'wp-mb_hms' ); ?></span>
			...
			<?php if ( $max_guests > 1 ) : ?>
				...
				<div class="widget-rooms-filter__group widget-rooms-filter__group--guests">
					...
					<span class="widget-rooms-filter__group-label"><?php esc_html_e( 'Guests', 'wp-mb_hms' ); ?></span>
			...
			<?php if ( $max_children > 0 ) : ?>
				...
				<div class="widget-rooms-filter__group widget-rooms-filter__group--children">
					...
					<span class="widget-rooms-filter__group-label"><?php esc_html_e( 'Children', 'wp-mb_hms' ); ?></span>
		...
		<?php
		// Extensions can hook into here to add their own options
		do_action( 'mb_hms_after_widget_rooms_filter', $default_filters );
