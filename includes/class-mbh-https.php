...
class MBH_HTTPS {
	...
	public static function init() {
		if ( mbh_get_option( 'enforce_ssl_booking' ) && ! is_admin() ) {
			...
			if ( mbh_get_option( 'unforce_ssl_booking' ) ) {
	...
	public static function force_https_url( $content ) {
		if ( is_ssl() ) {
			if ( is_array( $content ) ) {
				$content = array_map( 'MBH_HTTPS::force_https_url', $content );
			} else {
	...
	public static function force_https_page_link( $link, $page_id ) {
		if ( $page_id == get_option( 'mb_hms_booking_page_id' ) ) {
			$link = str_replace( 'http:', 'https:', $link );
		} elseif ( mbh_get_option( 'unforce_ssl_booking' ) ) {
	...
	public static function force_https_template_redirect() {
		if ( ! is_ssl() && ( is_booking() || apply_filters( 'mb_hms_enforce_ssl_booking', false ) ) ) {
	...
	public static function unforce_https_template_redirect() {
		...
		if ( is_ssl() && $_SERVER[ 'REQUEST_URI' ] && ! is_booking() && ! is_ajax() && apply_filters( 'mb_hms_unforce_ssl_booking', true ) ) {
