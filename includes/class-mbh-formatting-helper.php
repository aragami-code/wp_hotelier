...
if ( ! class_exists( 'MBH_Formatting_Helper' ) ) :

/**
 * MBH_Formatting_Helper Class
 */
class MBH_Formatting_Helper {
	/**
	...
	public static function is_valid_checkin_checkout( $checkin, $checkout, $force = false ) {
		...
		if ( ! $force ) {
			...
			$maximum_nights = apply_filters( 'mb_hms_booking_maximum_nights', mbh_get_option( 'booking_maximum_nights', 0 ) );
		}
		...
		if ( ! $force ) {
			...
			return apply_filters( 'mb_hms_is_valid_checkin_checkout', true, $checkin, $checkout );
		}
	...
	/**
	 * Sanitizes a string key for mb_hms Settings
	 * @param  string  $key
	 * @return string
	 */
