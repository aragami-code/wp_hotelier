...
class MBH_Comments {
	...
	public static function exclude_reservation_comments( $clauses ) {
		...
		if ( is_admin() && in_array( get_current_screen()->id, array( 'room_reservation', 'edit-room_reservation' ) ) && current_user_can( 'manage_mb_hms' ) ) {
	...
	public static function wp_count_comments( $stats, $post_id ) {
		...
		if ( 0 === $post_id ) {
			$stats = get_transient( 'mb_hms_count_comments' );
			...
			if ( ! $stats ) {
				...
				$stats = (object) $stats;
				set_transient( 'mb_hms_count_comments', $stats );
			}
		}
