...
class MBH_Cart {
	...
	public function __construct() {
		...
		add_action( 'mb_hms_add_to_cart', array( $this, 'calculate_totals' ), 20, 0 );
	}
	...
	public function init() {
		...
		add_action( 'mb_hms_booking_check_rooms_availability', array( $this, 'check_cart_items' ), 1 );
	}
	...
	public function get_cart() {
		if ( ! did_action( 'wp_loaded' ) ) {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Get cart should not be called before the wp_loaded action.', 'wp-mb_hms' ), '1.0.0' );
		}

		if ( ! did_action( 'mb_hms_cart_loaded_from_session' ) ) {
			$this->get_cart_from_session();
		}
	...
	public function get_cart_from_session() {
		...
		foreach ( $this->cart_session_data as $key => $default ) {
			$this->$key = MBH()->session->get( $key, $default );
		}

		$update_cart_session = false;

		// Load the cart object
		$cart = MBH()->session->get( 'cart', null );
		...
		$this->cart_contents_quantity = MBH()->session->get( 'cart_contents_quantity' );
		...
		if ( is_array( $cart ) ) {
			foreach ( $cart as $key => $values ) {
				$_room = mbh_get_room( $values[ 'room_id' ] );
				...
				if ( ! empty( $_room ) && $_room->exists() && $values[ 'quantity' ] > 0 ) {
					...
					if ( ! $_room->is_available( $this->checkin, $this->checkout, $values[ 'quantity' ] ) ) {
						...
						mbh_add_notice( sprintf( esc_html__( '%s has been removed from your cart because it can no longer be reserved. Please contact us if you need assistance.', 'wp-mb_hms' ), $_room->get_title() ), 'error' );
						do_action( 'mb_hms_remove_cart_item_from_session', $key, $values );

					} else {

						// Put session data into array. Run through filter so other plugins can load their own session data
						$session_data = array_merge( $values, array( 'data' => $_room ) );
						$this->cart_contents[ $key ] = apply_filters( 'mb_hms_get_cart_item_from_session', $session_data, $values, $key );
		...
		// Trigger action
		do_action( 'mb_hms_cart_loaded_from_session', $this );

		if ( $update_cart_session ) {
			MBH()->session->set( 'cart', $this->get_cart_for_session() );
		}
	...
	public function set_session() {
		...
		MBH()->session->set( 'cart', $cart_session );
		MBH()->session->set( 'cart_contents_quantity', $this->cart_contents_quantity );

		foreach ( $this->cart_session_data as $key => $default ) {
			MBH()->session->set( $key, $this->$key );
		}

		do_action( 'mb_hms_cart_updated' );
	}
	...
	public function empty_cart() {
		...
		MBH()->session->set( 'cart', null );
		MBH()->session->set( 'reservation_awaiting_payment', null );
		MBH()->session->set( 'cart_contents_quantity', null );

		do_action( 'mb_hms_cart_emptied' );
	}
	...
	*/
	...
	public function add_to_cart( $room_id = 0, $quantity = 1, $rate_id = 0 ) {
		...
		try {
			...
			if ( ! MBH_Formatting_Helper::is_valid_checkin_checkout( $this->checkin, $this->checkout ) ) {
				throw new Exception( esc_html__( 'Sorry, this room is not available on the given dates.', 'wp-mb_hms' ) );
			}
			...
			if ( ! $_room->exists() ) {
				...
				throw new Exception( esc_html__( 'Sorry, this room does not exists.', 'wp-mb_hms' ) );
			}
			...
			$this->cart_contents[ $cart_item_key ] = apply_filters( 'mb_hms_add_cart_item', array(
				'data'           => $_room,
				'room_id'        => $_room->id,
				'quantity'       => $quantity,
				'rate_id'        => $rate_id,
			...
			do_action( 'mb_hms_add_to_cart', $cart_item_key, $room_id, $quantity, $rate_id );
	...
	public function calculate_totals() {
		...
		do_action( 'mb_hms_before_calculate_totals', $this );
		...
		foreach ( $cart as $cart_item_key => $values ) {
			...
			if ( ! $line_price ) {
				...
				mbh_add_notice( esc_html__( 'Sorry, this room cannot be reserved.', 'wp-mb_hms' ), 'error' );
			}
			...
			$line_to_pay = apply_filters( 'mb_hms_line_to_pay', $line_to_pay, $line_price, $line_deposit, $qty, $room_type, $room_data );
		...
		// Subtotal
		$this->subtotal = apply_filters( 'mb_hms_calculated_subtotal', $this->cart_contents_total, $this );

		// Calculate taxes
		$this->tax_total        = mbh_is_tax_enabled() ? mbh_calculate_tax( $this->cart_contents_total ) : 0;

		// Taxes on deposits
		if ( mbh_is_deposit_tax_enabled() ) {
			$this->required_deposit = $this->required_deposit + mbh_calculate_tax( $this->required_deposit );
		}

		// Allow plugins to hook and alter totals before final total is calculated
		do_action( 'mb_hms_calculate_totals', $this );

		$total       = $this->cart_contents_total + mbh_calculate_tax( $this->cart_contents_total );
		$this->total = apply_filters( 'mb_hms_calculated_total', $total, $this );

		do_action( 'mb_hms_after_calculate_totals', $this );
	...
	public function needs_payment() {
		return apply_filters( 'mb_hms_cart_needs_payment', $this->required_deposit > 0, $this );
	}
	...
	public function generate_cart_item_key( $room_id, $rate_id ) {
		return mbh_generate_item_key( $room_id, $rate_id );
	}
	...
	private function reset() {
		foreach ( $this->cart_session_data as $key => $default ) {
			$this->$key = $default;
			MBH()->session->set( $key, $default );
		}

		do_action( 'mb_hms_cart_reset', $this );
	}
	...
	public function check_cart_items() {
		...
		if ( is_wp_error( $result ) ) {
			mbh_add_notice( $result->get_error_message(), 'error' );
			$return = false;
		}
	...
	public function check_cart_item_validity() {
		foreach ( $this->get_cart() as $cart_item_key => $values ) {
			...
			if ( ! $_room || ! $_room->exists() || $_room->post->post_status == 'trash' ) {
				...
				return new WP_Error( 'invalid', sprintf( esc_html__( '%s has been removed from your cart because it can no longer be reserved. Please contact us if you need assistance.', 'wp-mb_hms' ), $_room->get_title() ) );
			}
		}
	...
	public function check_cart_item_availability() {
		...
		foreach ( $this->get_cart() as $cart_item_key => $values ) {
			...
			if ( ! $_room->is_available( $this->checkin, $this->checkout, $qty ) ) {
				$error->add( 'room-not-available', sprintf( esc_html__( 'Sorry, we do not have enough "%s" available to fulfill your reservation. Please select another room or reduce the quantity and try again. We apologise for any inconvenience caused.', 'wp-mb_hms' ), $_room->get_title() ) );
	...
	public function check_cart_item_for_extensions() {
		foreach ( $this->get_cart() as $cart_item_key => $values ) {
			...
			if ( ! apply_filters( 'mb_hms_check_cart_item_for_extensions', true, $_room, $this->checkin, $this->checkout, $qty, $values ) ) {
				return new WP_Error( 'invalid', esc_html( apply_filters( 'mb_hms_check_cart_item_for_extensions_error', __( 'Sorry, this room cannot be reserved.', 'wp-mb_hms' ), $_room, $this->checkin, $this->checkout, $qty, $values ) ) );
			}
		}
	...
	public function set_quantity( $cart_item_key, $quantity = 1 ) {
		if ( $quantity == 0 || $quantity < 0 ) {
			do_action( 'mb_hms_before_cart_item_quantity_zero', $cart_item_key );
			...
			do_action( 'mb_hms_after_cart_item_quantity_update', $cart_item_key, $quantity, $old_quantity );
		}
	...
	public function get_total() {
		return apply_filters( 'mb_hms_cart_total', $this->total );
	}
	...
	public function get_subtotal() {
		return apply_filters( 'mb_hms_cart_subtotal', $this->subtotal );
	}
	...
	public function get_tax_total() {
		return apply_filters( 'mb_hms_cart_tax_total', $this->tax_total );
	}
	...
	public function get_required_deposit() {
		return apply_filters( 'mb_hms_cart_required_deposit', $this->required_deposit );
	}
	...
	public function get_room_price( $total ) {
		$total = mbh_price( mbh_convert_to_cents( $total ) );

		return apply_filters( 'mb_hms_cart_room_price', $total );
	}
	...
	public function get_booking_form_url() {
		$booking_form_url = mbh_get_page_permalink( 'booking' );
		if ( $booking_form_url ) {
			// Force SSL if needed
			if ( is_ssl() || mbh_get_option( 'enforce_ssl_booking' ) ) {
				$booking_form_url = str_replace( 'http:', 'https:', $booking_form_url );
			}
		}
		return apply_filters( 'mb_hms_get_booking_form_url', $booking_form_url );
	}
	...
	public function get_room_list_form_url( $room_id = false ) {
		$room_list_form_url = mbh_get_page_permalink( 'listing' );
		...
		return apply_filters( 'mb_hms_get_room_list_form_url', $room_list_form_url );
	}
