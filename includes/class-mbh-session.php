...
class MBH_Session {
	...
	public function __construct() {
		...
		$this->_cookie = 'wp_mb_hms_session_' . COOKIEHASH;
		$this->_table  = $wpdb->prefix . 'mb_hms_sessions';
		...
		// Actions
		add_action( 'mb_hms_set_cookies', array( $this, 'set_guest_session_cookie' ), 10 );
		add_action( 'mb_hms_cleanup_sessions', array( $this, 'cleanup_sessions' ), 10 );
		add_action( 'shutdown', array( $this, 'save_data' ), 20 );
		add_action( 'wp_logout', array( $this, 'destroy_session' ) );

		if ( ! is_user_logged_in() ) {
			add_action( 'mb_hms_received', array( $this, 'destroy_session' ) );
	...
	public function set_guest_session_cookie( $set ) {
		if ( $set ) {
			...
			// Set the cookie
			mbh_setcookie( $this->_cookie, $cookie_value, $this->_session_expiration, apply_filters( 'mb_hms_session_use_secure_cookie', false ) );
		}
	}
	...
	public function set_session_expiration() {
		$this->_session_expiring   = time() + intval( apply_filters( 'mb_hms_session_expiring', 60 * 60 * 47 ) ); // 47 Hours.
		$this->_session_expiration = time() + intval( apply_filters( 'mb_hms_session_expiration', 60 * 60 * 48 ) ); // 48 Hours.
	}
	...
	private function get_cache_prefix() {
		$prefix_num = wp_cache_get( 'mb_hms_session_prefix', MBH_SESSION_CACHE_GROUP );

		if ( $prefix_num === false ) {
			$prefix_num = 1;
			wp_cache_set( 'mb_hms_session_prefix', $prefix_num, MBH_SESSION_CACHE_GROUP );
		}

		return 'mb_hms_session_' . $prefix_num . '_';
	}
	...
	public function save_data() {
		...
		if ( $this->_dirty && $this->has_session() ) {
			...
			// Set cache
			wp_cache_set( $this->get_cache_prefix() . $this->_guest_id, $this->_data, MBH_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
	...
	public function destroy_session() {
		// Clear cookie
		mbh_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, apply_filters( 'mb_hms_session_use_secure_cookie', false ) );
	...
	public function cleanup_sessions() {
		...
		if ( ! defined( 'WP_SETUP_CONFIG' ) && ! defined( 'WP_INSTALLING' ) ) {
			...
			// Invalidate cache
			wp_cache_incr( 'mb_hms_session_prefix', 1, MBH_SESSION_CACHE_GROUP );
		}
	}
	...
	public function get_session( $guest_id, $default = false ) {
		...
		$value = wp_cache_get( $this->get_cache_prefix() . $guest_id, MBH_SESSION_CACHE_GROUP );
		...
		if ( false === $value ) {
			...
			wp_cache_add( $this->get_cache_prefix() . $guest_id, $value, MBH_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
		}
	...
	public function delete_session( $guest_id ) {
		...
		wp_cache_delete( $this->get_cache_prefix() . $guest_id, MBH_SESSION_CACHE_GROUP );
