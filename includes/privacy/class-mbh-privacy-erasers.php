...
if ( ! class_exists( 'MBH_Privacy_Erasers' ) ) :

/**
 * MBH_Privacy_Erasers Class
 */
class MBH_Privacy_Erasers {
	...
	public static function reservation_data_eraser( $email ) {
		$erasure_enabled = mbh_get_option( 'privacy_remove_reservation_data_on_erasure_request', false );
		$response        = array(
			'items_removed'  => false,
		...
		foreach ( $reservations as $reservation ) {
			if ( apply_filters( 'mb_hms_privacy_erase_reservation_personal_data', $erasure_enabled, $reservation ) ) {
				self::remove_reservation_personal_data( $reservation );

				$response[ 'messages' ][]    = sprintf( __( 'Removed personal data from reservation %s.', 'wp-mb_hms' ), $reservation->get_reservation_number() );
				$response[ 'items_removed' ] = true;
			} else {
				$response[ 'messages' ][]     = sprintf( __( 'Personal data within reservation %s has been retained.', 'wp-mb_hms' ), $reservation->get_reservation_number() );
	...
	public static function remove_reservation_personal_data( $reservation ) {
		 ...
		 *
		 * @param mbh_Reservation $reservation A reservation object.
		 */
		do_action( 'mb_hms_privacy_before_remove_reservation_personal_data', $reservation );

		/**
		 * Expose props and data types we'll be anonymizing.
		 */
		$props_to_remove = apply_filters( 'mb_hms_privacy_remove_reservation_personal_data_props', array(
		...
		if ( ! empty( $props_to_remove ) && is_array( $props_to_remove ) ) {
			foreach ( $props_to_remove as $prop => $data_type ) {
				...
				$anon_value = apply_filters( 'mb_hms_privacy_remove_reservation_personal_data_prop_value', $anon_value, $prop, $value, $data_type, $reservation );
		...
		// Remove meta data.
		$meta_to_remove = apply_filters( 'mb_hms_privacy_remove_reservation_personal_data_meta', array(
		...
		if ( ! empty( $meta_to_remove ) && is_array( $meta_to_remove ) ) {
			foreach ( $meta_to_remove as $meta_key => $data_type ) {
				...
				$anon_value = apply_filters( 'mb_hms_privacy_remove_reservation_personal_data_meta_value', $anon_value, $meta_key, $value, $data_type, $order );
		...
		$reservation->add_reservation_note( esc_html__( 'Personal data removed.', 'wp-mb_hms' ) );

		/**
		 * Allow extensions to remove their own personal data for this reservation.
		 */
		do_action( 'mb_hms_privacy_remove_reservation_personal_data', $reservation );
	}
}
