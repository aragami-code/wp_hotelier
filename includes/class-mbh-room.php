...
class MBH_Room {
	...
	public function __construct( $room ) {
		...
		} elseif ( $room instanceof MBH_Room ) {
	...
	public function is_visible( $show_out_of_stock = false ) {
		...
		return apply_filters( 'mb_hms_room_is_visible', $visible, $this->id );
	}
	...
	public function get_title() {
		...
		return apply_filters( 'mb_hms_room_title', $title, $this );
	}
	...
	public function get_gallery_attachment_ids() {
		return apply_filters( 'mb_hms_room_gallery_attachment_ids', array_filter( (array) explode( ',', $this->room_image_gallery ) ), $this );
	}
	...
	public function get_stock_rooms() {
		return absint( apply_filters( 'mb_hms_get_stock_rooms', $this->stock_rooms, $this->id ) );
	}
	...
	public function get_available_rooms( $checkin, $checkout = false ) {
		...
		return apply_filters( 'mb_hms_get_available_rooms', $available_rooms, $this );
	}
	...
	public function get_min_nights() {
		return apply_filters( 'mb_hms_per_room_minimum_nights', mbh_get_option( 'booking_minimum_nights', 1 ), $this->id );
	}
	...
	public function get_max_nights() {
		return apply_filters( 'mb_hms_per_room_maximum_nights', mbh_get_option( 'booking_maximum_nights', 0 ), $this->id );
	}
	...
	public function has_enough_rooms( $checkin, $checkout, $qty ) {
		...
		return apply_filters( 'mb_hms_room_has_enough_rooms', $has_enough, $checkin, $checkout, $qty, $this->id );
	}
	...
	public function is_available( $checkin, $checkout = false, $qty = 1, $rate_id = false ) {
		...
		return apply_filters( 'mb_hms_room_is_available', $is_available, $this->id, $checkin, $checkout );
	}
	...
	public function get_reserved_rooms( $checkin, $checkout ) {
		...
		return apply_filters( 'mb_hms_get_reserved_rooms', $count, $this );
	}
	...
	public function get_pending_rooms() {
		...
		$reservation_id = null !== MBH()->session ? absint( MBH()->session->get( 'reservation_awaiting_payment' ) ) : 0;

		if ( ! $reservation_id > 0 ) {
			$count = 0;
		} else {
			$sql      = $wpdb->prepare( "SELECT room_id FROM {$wpdb->prefix}mb_hms_rooms_bookings rb, {$wpdb->prefix}mb_hms_bookings b WHERE b.status = 'pending' OR b.status = 'failed' AND rb.reservation_id = b.reservation_id AND b.reservation_id = %d AND rb.room_id = %d", $reservation_id, $this->id  );
			$result   = $wpdb->get_results( $sql );
			$count    = count( $result );
		}

		return apply_filters( 'mb_hms_get_pending_rooms', $count, $this );
	}
	...
	public function get_formatted_room_size() {
		$formatted_room_size = $this->room_size ? $this->room_size . ' ' . mbh_get_option( 'room_size_unit', 'm²' ) : '';

		return apply_filters( 'mb_hms_room_size', $formatted_room_size, $this );
	}
	...
	public function get_regular_price( $checkin, $checkout = false ) {
		...
		$price = apply_filters( 'mb_hms_get_regular_price', $price, $checkin, $checkout, $this );
	...
	public function get_sale_price( $checkin, $checkout = false ) {
		...
		$price = apply_filters( 'mb_hms_get_sale_price', $price, $checkin, $checkout, $this );
	...
	public function get_seasonal_price( $checkin, $checkout = false ) {
		...
		$price = 0;
		$rules = mbh_get_seasonal_prices_schema();
		...
		if ( $price > 0 ) {
			return apply_filters( 'mb_hms_get_seasonal_price', $price, $this );
	...
	public function is_on_sale( $checkin, $checkout = false ) {
		...
		return apply_filters( 'mb_hms_room_is_on_sale', $is_on_sale, $checkin, $checkout, $this );
	}
	...
	public function get_price( $checkin, $checkout = false ) {
		...
		return apply_filters( 'mb_hms_get_price', $price, $checkin, $checkout, $this );
	}
	...
	public function get_price_html( $checkin, $checkout ) {
		if ( $this->is_variable_room() ) {
			...
			if ( $min_price && $min_price > 0 ) {
				...
				$min_price = sprintf( _x( 'From %s', 'min_price', 'wp-mb_hms' ), mbh_price( $min_price ) );
				$price     = apply_filters( 'mb_hms_get_rate_price_html', $min_price, $this );

			} else {

				$price = apply_filters( 'mb_hms_empty_price_html', '', $this );
			...
			if ( $this->is_on_sale( $checkin, $checkout ) ) {
				...
				$price = sprintf( _x( 'Price: %s', 'price', 'wp-mb_hms' ), $this->get_price_html_from_to( $from, $to ) );

				$price = apply_filters( 'mb_hms_sale_price_html', $price, $this );

			} else {

				$price = mbh_price( $get_price / 100 ); // (prices are stored as integers)
				$price = sprintf( _x( 'Price: %s', 'price', 'wp-mb_hms' ), $price );
				$price = apply_filters( 'mb_hms_get_price_html', $price, $this );
	...
	public function get_price_html_from_to( $from, $to ) {
		$price = '<del>' . ( ( is_numeric( $from ) ) ? mbh_price( $from ) : $from ) . '</del> <ins>' . ( ( is_numeric( $to ) ) ? mbh_price( $to ) : $to ) . '</ins>';

		return apply_filters( 'mb_hms_get_price_html_from_to', $price, $from, $to, $this );
	}
	...
	public function get_min_price_html() {
		...
		if ( $this->is_variable_room() ) {
			...
			if ( $min_price > 0 ) {
				$min_price = sprintf( __( 'Rates from %s per night', 'wp-mb_hms' ), mbh_price( $min_price ) );
			} else {
				$min_price = apply_filters( 'mb_hms_empty_price_html', '', $this );
			}
			...
			if ( $this->has_seasonal_price() ) {
				...
				if ( $min_price > 0 ) {
					if ( $this->sale_price && $this->regular_price ) {
						...
						$min_price = sprintf( __( '%s per night', 'wp-mb_hms' ), $this->get_price_html_from_to( $from, $to ) );
					} else {
						$min_price = sprintf( __( '%s per night', 'wp-mb_hms' ), mbh_price( $min_price ) );
					}
				} else {
					$min_price = apply_filters( 'mb_hms_empty_price_html', '', $this );
				}
		...
		$min_price = apply_filters( 'mb_hms_min_price_html', $min_price, $this );
	...
	public function get_room_conditions() {
		...
		return apply_filters( 'mb_hms_room_conditions', $room_conditions, $this );
	}
	...
	public function get_deposit() {
		...
		return apply_filters( 'mb_hms_room_deposit', $percentage, $this );
	}
	...
	public function get_formatted_deposit() {
		...
		return apply_filters( 'mb_hms_room_formatted_deposit', $percentage, $this );
	}
	...
	public function get_image( $size = 'room_thumbnail', $attr = array() ) {
		if ( has_post_thumbnail( $this->id ) ) {
			...
			$image = mbh_placeholder_img( $size );
		}
	 ...
	 * @return mbh_Room_Variation on success, false on failure
	 */
	public function get_room_variation( $rate_id ) {
		...
		return new MBH_Room_Variation( $variation, $this->id );
	}
