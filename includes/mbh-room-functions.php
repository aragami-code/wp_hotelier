 ...
 * @return mbh_Room
 */
function mbh_get_room( $the_room = false ) {
	return new MBH_Room( $the_room );
}
 ...
 * @return string
 */
function mbh_placeholder_img_src() {
	return apply_filters( 'mb_hms_placeholder_img_src', MBH()->plugin_url() . '/assets/images/placeholder.png' );
}
 ...
 * @return string
 */
function mbh_placeholder_img( $size = 'room_thumbnail' ) {
	$dimensions = mbh_get_image_size( $size );

	return apply_filters( 'mb_hms_placeholder_img', '<img src="' . mbh_placeholder_img_src() . '" alt="' . esc_attr__( 'Placeholder', 'wp-mb_hms' ) . '" width="' . esc_attr( $dimensions[ 'width' ] ) . '" class="mb_hms-placeholder wp-post-image" height="' . esc_attr( $dimensions[ 'height' ] ) . '" />', $size, $dimensions );
}
 ...
 * @return array.
 */
function mbh_get_room_ids() {

	// Load from cache
	$room_ids = get_transient( 'mb_hms_room_ids' );
	...
	set_transient( 'mb_hms_room_ids', $rooms, DAY_IN_SECONDS * 30 );
 ...
 */
function mbh_get_room_ids_unavailable( $checkin, $checkout, $args = array() ) {

	// Check if we want to show also the unavailable rooms
	if ( mbh_get_option( 'room_unavailable_visibility', false ) ) {
		return array();
	}
	...
	try {
		...
		if ( ! MBH_Formatting_Helper::is_valid_checkin_checkout( $checkin, $checkout ) ) {
			throw new Exception( esc_html__( 'Please check your dates.', 'wp-mb_hms' ) );
		}

		// query already reserved rooms and check if there is enough stock
		$sql            = $wpdb->prepare( "SELECT room_id, count(room_id) AS count FROM {$wpdb->prefix}mb_hms_rooms_bookings rb, {$wpdb->prefix}mb_hms_bookings b WHERE rb.reservation_id = b.reservation_id AND (%s < b.checkout AND %s > b.checkin) GROUP by room_id", $checkin, $checkout );
		...
		if ( ! empty( $reserved_rooms ) ) {
			foreach ( $reserved_rooms as $id => $row ) {
				$_room = mbh_get_room( $row[ 'room_id' ] );
		...
		return apply_filters( 'mb_hms_get_room_ids_unavailable', $ids );
 ...
 * @return array.
 */
function mbh_get_available_room_ids( $checkin, $checkout ) {
	$all_rooms         = mbh_get_room_ids(); // get all rooms
	$unavailable_rooms = mbh_get_room_ids_unavailable( $checkin, $checkout ); // get unavailable rooms
	...
	foreach ( $filtered_ids as $id ) {
		$_room = mbh_get_room( $id );

		if ( $_room->exists() ) {
			if ( mbh_get_option( 'room_unavailable_visibility', false ) || $_room->is_available( $checkin, $checkout ) ) {
				$ids[] = $id;
			}
		}
	}

	return apply_filters( 'mb_hms_get_available_room_ids', $ids, $checkin, $checkout );
}
 ...
 * @return array.
 */
function mbh_get_related_rooms_query( $room_id, $limit, $order ) {
	...
	$args = apply_filters( 'mb_hms_related_rooms_args', array(
		'post_type'            => 'room',
		'ignore_sticky_posts'  => 1,
		'no_found_rows'        => 1,
 ...
 * @return array.
 */
function mbh_get_listing_rooms_query( $checkin, $checkout, $room_id = false ) {
	// Get available rooms
	$room_ids = mbh_get_available_room_ids( $checkin, $checkout );
	...
	// Get sorting option
	$listing_sorting = mbh_get_option( 'listing_sorting', 'menu_order' );
	...
	$rooms = new WP_Query( apply_filters( 'mb_hms_shortcode_room_list_query', $query_args ) );
 ...
 * @return string
 */
function mbh_get_list_of_rooms_html( $name ) {
 ...
 * @return string
 */
function mbh_get_room_price_breakdown( $checkin, $checkout, $room_id, $rate_id, $qty ) {
	$_room        = mbh_get_room( $room_id );
 ...
 */
function mbh_get_room_reservations( $room_id, $checkin, $checkout ) {
	global $wpdb;

	$sql          = $wpdb->prepare( "SELECT rb.reservation_id, b.checkin, b.checkout, b.status FROM {$wpdb->prefix}mb_hms_rooms_bookings rb, {$wpdb->prefix}mb_hms_bookings b WHERE rb.reservation_id = b.reservation_id AND rb.room_id = %d AND (%s <= b.checkout AND %s >= b.checkin) ORDER BY b.checkin", $room_id, $checkin, $checkout );
 ...
 * @return string
 */
function mbh_get_room_min_max_info( $min_nights, $max_nights ) {
	if ( $min_nights > 1 && $max_nights ) {
		$text = sprintf( __( '%s nights minimum stay and %s nights maximum stay', 'wp-mb_hms' ), absint( $min_nights ), absint( $max_nights ) );
	} else if ( $min_nights > 1 ) {
		$text = sprintf( __( '%s nights minimum stay', 'wp-mb_hms' ), absint( $min_nights ) );
	} else if ( $max_nights ) {
		$text = sprintf( _n( '%s night maximum stay', '%s nights maximum stay', $max_nights, 'wp-mb_hms' ), absint( $max_nights ) );
	} else {
		$text = '';
	}
