...
if ( ! class_exists( 'MBH_Reservation' ) ) :

/**
 * MBH_Reservation Class
 */
class MBH_Reservation {
	/**
	...
	public function __construct( $reservation ) {
		...
		} elseif ( $reservation instanceof MBH_Reservation ) {
	...
	public function get_reservation_number() {
		return apply_filters( 'mb_hms_reservation_number', $this->id, $this );
	}
	 ...
	 *
	 * @param MBH_Payment_Gateway $payment_method
	 */
	...
	public function get_address() {
		...
		return apply_filters( 'mb_hms_get_guest_address', $address, $this );
	}
	...
	public function get_formatted_guest_address() {
		...
		return apply_filters( 'mb_hms_get_formatted_guest_address', $address, $this );
	}
	...
	public function get_arrival_time() {
		return apply_filters( 'mb_hms_get_guest_arrival_time', intval( $this->guest_arrival_time ), $this );
	}
	...
	public function get_formatted_arrival_time() {
		...
		$formatted_time = esc_html__( 'I don\'t know', 'wp-mb_hms' );
		...
		return apply_filters( 'mb_hms_get_guest_formatted_arrival_time', $formatted_time, $this );
	}
	...
	public function get_guest_special_requests() {
		...
		return apply_filters( 'mb_hms_get_guest_special_requests', $message, $this );
	}
	...
	/**
	 * Return the reservation statuses without mbh- internal prefix.
	 *
	...
	public function get_status() {
		...
		return apply_filters( 'mb_hms_reservation_get_status', 'mbh-' === substr( $this->post_status, 0, 4 ) ? substr( $this->post_status, 4 ) : $this->post_status, $this );
	}
	...
	public function has_status( $status ) {
		return apply_filters( 'mb_hms_reservation_has_status', ( is_array( $status ) && in_array( $this->get_status(), $status ) ) || $this->get_status() === $status ? true : false, $this, $status );
	}
	...
	public function add_reservation_note( $note ) {
		$comment_author_email = strtolower( esc_html__( 'mb_hms', 'wp-mb_hms' ) ) . '@';
		$comment_author_email .= isset( $_SERVER['HTTP_HOST'] ) ? str_replace( 'www.', '', $_SERVER['HTTP_HOST'] ) : 'noreply.com';
		$comment_author_email = sanitize_email( $comment_author_email );

		$commentdata = apply_filters( 'mb_hms_new_reservation_note_data', array(
			'comment_post_ID' => $this->id,
			'comment_author' => esc_html__( 'mb_hms', 'wp-mb_hms' ),
			'comment_author_email' => $comment_author_email,
			'comment_author_url' => '',
			'comment_content' => esc_html( $note ),
			'comment_agent' => 'mb_hms',
	 ...
	 *
	 * @param string $new_status Status to change the reservation to. No internal mbh- prefix is required.
	...
	public function update_status( $new_status, $note = '',  $manual = false ) {
		...
		$new_status = 'mbh-' === substr( $new_status, 0, 4 ) ? substr( $new_status, 4 ) : $new_status;
		$old_status = $this->get_status();

		// Only update if they differ - and ensure post_status is a 'mbh' status.
		if ( $new_status !== $old_status || ! in_array( $this->post_status, array_keys( mbh_get_reservation_statuses() ) ) ) {
			...
			if ( $old_status == 'cancelled' || $old_status == 'completed' || $old_status == 'refunded' ) {
				...
				$this->add_reservation_note( trim( $note . ' ' . sprintf( esc_html__( 'Error: Trying to change the status from %1$s to %2$s. %1$s reservations cannot be restored or modified.', 'wp-mb_hms' ), mbh_get_reservation_status_name( $old_status ), mbh_get_reservation_status_name( $new_status ) ) ) );

			} else {
				// Temporarily remove reservation save action to
				// avoid email triggering twice
				remove_action( 'mb_hms_process_room_reservation_meta', 'MBH_Meta_Box_Reservation_Save::save', 30, 2 );

				// Update the reservation
				wp_update_post( array( 'ID' => $this->id, 'post_status' => 'mbh-' . $new_status ) );

				// Re-enable save action
				add_action( 'mb_hms_process_room_reservation_meta', 'MBH_Meta_Box_Reservation_Save::save', 30, 2 );

				$this->post_status = 'mbh-' . $new_status;

				// Update the reservation table
				$this->update_table_status( $new_status );

				$this->add_reservation_note( trim( $note . ' ' . sprintf( esc_html__( 'Reservation status changed from %s to %s.', 'wp-mb_hms' ), mbh_get_reservation_status_name( $old_status ), mbh_get_reservation_status_name( $new_status ) ) ) );

				// Status was changed
				do_action( 'mb_hms_reservation_status_' . $new_status, $this->id );

				if ( $manual ) {
					do_action( 'mb_hms_reservation_status_' . $old_status . '_to_' . $new_status . '_manual', $this->id );
				} else {
					do_action( 'mb_hms_reservation_status_' . $old_status . '_to_' . $new_status, $this->id );
				}

				do_action( 'mb_hms_reservation_status_changed', $this->id, $old_status, $new_status );
			}
		}
	}

	/**
	 * Update reservation status in table 'mb_hms_bookings'.
	 *
	...
	public function add_item( $room, $qty = 1, $args = array() ) {
		...
		$item_id = mbh_add_reservation_item( $this->id, array(
		...
		mbh_add_reservation_item_meta( $item_id, '_qty', absint( $qty ) );
		mbh_add_reservation_item_meta( $item_id, '_room_id', absint( $room->id ) );

		if ( $args[ 'rate_name' ] ) {
			mbh_add_reservation_item_meta( $item_id, '_rate_name', $args[ 'rate_name' ] );
		}

		mbh_add_reservation_item_meta( $item_id, '_max_guests', absint( $args[ 'max_guests' ] ? $args[ 'max_guests' ] : 0 ) );
		mbh_add_reservation_item_meta( $item_id, '_price', absint( $args[ 'price' ] ? $args[ 'price' ] : 0 ) );
		mbh_add_reservation_item_meta( $item_id, '_total', absint( $args[ 'total' ] ? $args[ 'total' ] : 0 ) );
		mbh_add_reservation_item_meta( $item_id, '_percent_deposit', absint( $args[ 'percent_deposit' ] ? $args[ 'percent_deposit' ] : 0 ) );
		mbh_add_reservation_item_meta( $item_id, '_deposit', absint( $args[ 'deposit' ] ? $args[ 'deposit' ] : 0 ) );
		mbh_add_reservation_item_meta( $item_id, '_is_cancellable', absint( $args[ 'is_cancellable' ] ? $args[ 'is_cancellable' ] : false ) );

		$adults = $args[ 'adults' ] && is_array( $args[ 'adults' ] ) ? array_map( 'absint', $args[ 'adults' ] ) : false;
		mbh_add_reservation_item_meta( $item_id, '_adults', $adults );

		$children = $args[ 'children' ] && is_array( $args[ 'children' ] ) ? array_map( 'absint', $args[ 'children' ] ) : false;
		mbh_add_reservation_item_meta( $item_id, '_children', $children );

		do_action( 'mb_hms_reservation_add_item', $this->id, $item_id, $room, $qty, $args );
	...
	public function get_items() {
		...
		$items          = array();
		$get_items_sql  = $wpdb->prepare( "SELECT reservation_item_id, reservation_item_name FROM {$wpdb->prefix}mb_hms_reservation_items WHERE reservation_id = %d ", $this->id );
		...
		return apply_filters( 'mb_hms_reservation_get_items', $items, $this );
	}
	...
	public function has_meta( $reservation_item_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value, meta_id, reservation_item_id
			FROM {$wpdb->prefix}mb_hms_reservation_itemmeta WHERE reservation_item_id = %d
	...
	public function get_room_from_item( $item ) {
		if ( ! empty( $item[ 'room_id' ]  ) ) {
			$_room = mbh_get_room( $item[ 'room_id' ] );
		} else {
			$_room = false;
		}

		return apply_filters( 'mb_hms_get_room_from_item', $_room, $item, $this );
	}
	...
	public function remove_reservation_items() {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM itemmeta USING {$wpdb->prefix}mb_hms_reservation_itemmeta itemmeta INNER JOIN {$wpdb->prefix}mb_hms_reservation_items items WHERE itemmeta.reservation_item_id = items.reservation_item_id and items.reservation_id = %d", $this->id ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_reservation_items WHERE reservation_id = %d", $this->id ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_rooms_bookings WHERE reservation_id = %d", $this->id ) );
		// $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_bookings WHERE reservation_id = %d", $this->id ) );
	}
	...
	public function get_formatted_line_total( $item ) {
		...
		$total = mbh_price( mbh_convert_to_cents( $item[ 'total' ] ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_reservation_formatted_line_total', $total, $item, $this );
	}
	...
	public function get_item_deposit( $item ) {
		...
		return apply_filters( 'mb_hms_get_item_deposit', $amount, $this );
	}
	...
	public function get_total() {
		return absint( apply_filters( 'mb_hms_reservation_amount_total', $this->reservation_total, $this ) );
	}
	...
	public function get_formatted_total() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_total() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_total', $amount, $this );
	}
	...
	public function mark_as_paid() {
		...
		$this->add_reservation_note( esc_html__( 'Reservation marked as paid.', 'wp-mb_hms' ) );
	...
	public function mark_as_unpaid() {
		...
		$this->add_reservation_note( esc_html__( 'Reservation marked as unpaid.', 'wp-mb_hms' ) );
	...
	public function get_deposit() {
		return absint( apply_filters( 'mb_hms_reservation_amount_deposit', $this->reservation_deposit, $this ) );
	}
	...
	public function get_formatted_deposit() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_deposit() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_deposit', $amount, $this );
	}
	...
	public function get_paid_deposit() {
		return absint( apply_filters( 'mb_hms_reservation_paid_deposit', $this->reservation_paid_deposit, $this ) );
	}
	...
	public function get_formatted_paid_deposit() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_paid_deposit() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_paid_deposit', $amount, $this );
	}
	...
	public function get_remain_deposit_charge() {
		...
		return apply_filters( 'mb_hms_reservation_remain_deposit_charge', $remain_deposit_charge, $this );
	}
	...
	public function get_formatted_remain_deposit_charge() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_remain_deposit_charge() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_remain_deposit_charge', $amount, $this );
	}
	...
	public function get_subtotal() {
		return absint( apply_filters( 'mb_hms_reservation_amount_subtotal', $this->reservation_subtotal, $this ) );
	}
	...
	public function get_formatted_subtotal() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_subtotal() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_subtotal', $amount, $this );
	}
	...
	public function get_tax_total() {
		return absint( apply_filters( 'mb_hms_reservation_amount_tax_total', $this->reservation_tax_total, $this ) );
	}
	...
	public function get_formatted_tax_total() {
		$amount = mbh_price( mbh_convert_to_cents( $this->get_tax_total() ), $this->get_reservation_currency() );

		return apply_filters( 'mb_hms_get_formatted_reservation_tax_total', $amount, $this );
	}
	...
	public function get_balance_due() {
		...
		return apply_filters( 'mb_hms_reservation_balance_due', $amount, $this );
	}
	...
	public function get_formatted_balance_due() {
		if ( $this->is_marked_as_paid() ) {
			$amount = '<del>' . mbh_price( mbh_convert_to_cents( $this->get_total() - $this->get_paid_deposit() - $this->get_remain_deposit_charge() ), $this->get_reservation_currency() ) . '</del> <ins>' . mbh_price( mbh_convert_to_cents( $this->get_balance_due() ), $this->get_reservation_currency() ) . '</ins>';
		} else {
			$amount = mbh_price( mbh_convert_to_cents( $this->get_balance_due() ), $this->get_reservation_currency() );
		}


		return apply_filters( 'mb_hms_get_formatted_reservation_balance_due', $amount, $this );
	}
	...
	public function get_reservation_totals() {
		...
		if ( $this->has_tax() ) {
			...
			$total_rows[ 'subtotal' ] = array(
				'label' => esc_html__( 'Subtotal:', 'wp-mb_hms' ),
				'value'	=> $this->get_formatted_subtotal()
			);

			$total_rows[ 'tax_total' ] = array(
				'label' => esc_html__( 'Tax total:', 'wp-mb_hms' ),
		...
		if ( $this->has_room_with_deposit() ) {
			...
			if ( $this->get_formatted_paid_deposit() > 0 ) {
				...
				$total_rows[ 'paid_deposit' ] = array(
					'label' => esc_html__( 'Paid Deposit:', 'wp-mb_hms' ),
				...
				$total_rows[ 'required_deposit' ] = array(
					'label' => esc_html__( 'Required Deposit:', 'wp-mb_hms' ),
			...
			if ( $this->get_paid_deposit() > 0 && $this->payment_method_title ) {
				$total_rows[ 'payment_method' ] = array(
					'label' => esc_html__( 'Payment Method:', 'wp-mb_hms' ),
		...
		$total_rows[ 'total' ] = array(
			'label' => esc_html__( 'Total Due:', 'wp-mb_hms' ),
			'value'	=> $this->get_formatted_balance_due()
		);

		return apply_filters( 'mb_hms_get_reservation_totals', $total_rows, $this );
	}
	...
	public function get_totals_before_booking() {
		...
		if ( $this->has_room_with_deposit() ) {
			...
			$total_rows[ 'total' ] = array(
				'label' => esc_html__( 'Total:', 'wp-mb_hms' ),
				'value'	=> $this->get_formatted_total()
			);

			$total_rows[ 'deposit_due' ] = array(
				'label' => esc_html__( 'Deposit Due Now:', 'wp-mb_hms' ),
				'value'	=> $this->get_formatted_deposit()
			);
		...
		return apply_filters( 'mb_hms_get_totals_before_booking', $total_rows, $this );
	}
	...
	public function get_nights() {
		...
		return apply_filters( 'mb_hms_get_nights', $nights, $this );
	}
	...
	public function get_checkin() {
		return apply_filters( 'mb_hms_get_checkin', $this->guest_checkin, $this );
	}
	...
	public function get_formatted_checkin() {
		...
		return apply_filters( 'mb_hms_get_checkin', $date, $this );
	}
	...
	public function get_checkout() {
		return apply_filters( 'mb_hms_get_checkout', $this->guest_checkout, $this );
	}
	...
	public function get_formatted_checkout() {
		...
		return apply_filters( 'mb_hms_get_checkout', $date, $this );
	}
	...
	public function get_reservation_currency() {
		return apply_filters( 'mb_hms_get_reservation_currency', $this->reservation_currency, $this );
	}
	...
	public function get_booking_payment_url( $on_booking = false ) {
		...
		$pay_url = mbh_get_endpoint_url( 'pay-reservation', $this->id, mbh_get_page_permalink( 'booking' ) );

		if ( mbh_get_option( 'enforce_ssl_booking' ) || is_ssl() ) {
		...
		return apply_filters( 'mb_hms_get_booking_payment_url', $pay_url, $this );
	}
	...
	public function get_booking_received_url() {
		$booking_received_url = mbh_get_endpoint_url( 'reservation-received', $this->id, mbh_get_page_permalink( 'booking' ) );
		...
		return apply_filters( 'mb_hms_get_booking_received_url', $booking_received_url, $this );
	}
	...
	public function get_booking_cancel_url( $redirect = '' ) {
		...
		return apply_filters( 'mb_hms_get_booking_cancel_url', wp_nonce_url( add_query_arg( array(
			'cancel_reservation' => 'true',
			'reservation'        => $this->reservation_key,
			'reservation_id'     => $this->id,
			'redirect'           => $redirect
		), $cancel_endpoint ), 'mb_hms-cancel_reservation' ) );
	}
	...
	public function get_cancel_endpoint() {
		$cancel_endpoint = mbh_get_page_permalink( 'listing' );
	...
	public function payment_complete( $transaction_id = '' ) {
		do_action( 'mb_hms_pre_payment_complete', $this->id );

		if ( null !== MBH()->session ) {
			MBH()->session->set( 'reservation_awaiting_payment', null );
		}

		$valid_reservation_statuses = apply_filters( 'mb_hms_valid_reservation_statuses_for_payment_complete', array( 'on-hold', 'pending', 'failed' ), $this );

		if ( $this->id && $this->has_status( $valid_reservation_statuses ) ) {

			$this->set_paid_deposit( $this->get_deposit() );

			$this->update_status( apply_filters( 'mb_hms_payment_complete_reservation_status', 'confirmed', $this->id ) );
			...
			do_action( 'mb_hms_payment_complete', $this->id );
		} else {
			do_action( 'mb_hms_payment_complete_reservation_status_' . $this->get_status(), $this->id );
		}
	}
	...
	public function send_request() {
		...
		if ( 'pending' == $status ) {
			...
			// Trigger emails
			do_action( 'mb_hms_new_booking_request', $this->id );
		}
	}
	...
	public function needs_payment() {
		$valid_reservation_statuses = apply_filters( 'mb_hms_valid_reservation_statuses_for_payment', array( 'pending', 'failed' ), $this );
		...
		return apply_filters( 'mb_hms_reservation_needs_payment', $needs_payment, $this, $valid_reservation_statuses );
	}
	...
	public function email_reservation_items_table( $plain_text = false ) {
		...
		mbh_get_template( $template, array(
			'reservation'           => $this,
			'items'                 => $this->get_items()
		) );

		return apply_filters( 'mb_hms_email_reservation_items_table', ob_get_clean(), $this );
	}
