...
if ( ! class_exists( 'MBH_Admin_Post_Types' ) ) :

/**
 * MBH_Admin_Post_Types Class
 *
 * Handles the functionality on the edit post screen for mb_hms post types.
 */
class MBH_Admin_Post_Types {
	/**
	 * Constructor
	 */
	...
	private function includes() {
		include_once( 'meta-boxes/class-mbh-admin-meta-boxes.php' );
	}
	...
	public function room_columns( $existing_columns ) {
		...
		$columns               = array();
		$columns[ 'guests' ]   = esc_html__( 'Guests', 'wp-mb_hms' );
		$columns[ 'type' ]     = esc_html__( 'Room Type', 'wp-mb_hms' );
		$columns[ 'quantity' ] = esc_html__( 'Available Today', 'wp-mb_hms' );
		$columns[ 'date' ]     = esc_html__( 'Date', 'wp-mb_hms' );
		...
		switch ( $column ) {
			...
			case 'quantity' :
				...
				if ( $available_rooms > 0 ) {
					...
					echo sprintf( _n( '%s room left', '%s rooms left', absint( $available_rooms ), 'wp-mb_hms' ), $available_rooms );

					echo '</mark>';
				} else {
					echo '<mark class="not-left">' . esc_html__( 'No rooms left!', 'wp-mb_hms' ) . '</mark>';
				}
 				break;
	...
	public function reservation_columns( $existing_columns ) {
		...
		$columns                = array();
		$columns[ 'title' ]     = esc_html__( 'Guest', 'wp-mb_hms' );
		$columns[ 'nights' ]    = esc_html__( 'Nights', 'wp-mb_hms' );
		$columns[ 'check_in' ]  = esc_html__( 'Check-in', 'wp-mb_hms' );
		$columns[ 'check_out' ] = esc_html__( 'Check-out', 'wp-mb_hms' );
		$columns[ 'date' ]      = esc_html__( 'Date', 'wp-mb_hms' );
		$columns[ 'status' ]    = esc_html__( 'Status', 'wp-mb_hms' );
	...
	public function render_room_reservation_columns( $column ) {
		...
		if ( empty( $the_reservation ) || $the_reservation->id != $post->ID ) {
			$the_reservation = mbh_get_reservation( $post );
		}
		...
		switch ( $column ) {
			...
			case 'status' :
				echo '<span class="' . esc_html( $the_reservation->get_status() ) . ' hastip" title="' . esc_html( mbh_get_reservation_status_name( $the_reservation->get_status() ) ) . '">' . esc_html( $the_reservation->get_status() ) . '</span>';
				break;
	...
	public function untrash_reservation( $postid ) {
		...
		if ( get_post_type() === 'room_reservation' ) {
			...
			$reservation = mbh_get_reservation( $postid );
			...
			if ( ! $ret ) {
				...
				$reservation->update_status( 'cancelled', esc_html__( 'This reservation cannot be restored because one or more rooms are no longer available on those dates.', 'wp-mb_hms' ) );
			} else {
	...
	public function delete_reservation_items( $postid ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM itemmeta USING {$wpdb->prefix}mb_hms_reservation_itemmeta itemmeta INNER JOIN {$wpdb->prefix}mb_hms_reservation_items items WHERE itemmeta.reservation_item_id = items.reservation_item_id and items.reservation_id = %d", $postid ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_reservation_items WHERE reservation_id = %d", $postid ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_rooms_bookings WHERE reservation_id = %d", $postid ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}mb_hms_bookings WHERE reservation_id = %d", $postid ) );
	}

	/**
	 * Delete `mb_hms_room_ids` transient when the post is trashed.
	 */
