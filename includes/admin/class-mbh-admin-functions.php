...
if ( ! class_exists( 'MBH_Admin_Functions' ) ) :

/**
 * L_Admin_Functions Class
 */
class MBH_Admin_Functions {
	/**
	 * Get all mb_hms screen ids.
	 */
	public static function get_screen_ids() {
		$prefix = self::get_prefix_screen_id();

		$screen_ids   = array(
			'toplevel_page_mb_hms-settings',
			$prefix . '_mb_hms-logs',
			$prefix . '_mb_hms-add-reservation',
			$prefix . '_mb_hms-calendar',
			$prefix . '_mb_hms-addons',
			'edit-room',
			'room',
			'room_reservation',
		...
		return apply_filters( 'mb_hms_screen_ids', $screen_ids );
	}

	/**
	 * Helper to get the screen ID in case "mb_hms" is translated.
	 *
	 * See https://core.trac.wordpress.org/ticket/18857
	 */
	public static function get_prefix_screen_id() {

	    $prefix = sanitize_title( __( 'mb_hms', 'wp-mb_hms' ) );
	    return $prefix . '_page';
	}
	...
	public static function create_page( $key, $slug, $option = '', $page_title = '', $page_content = '', $post_parent = 0 ) {
		...
		$page_found = apply_filters( 'mb_hms_create_page_id', $page_found, $slug, $page_content );
		...
		if ( $page_found && ! $page_found_trash ) {
			if ( ! $option_value ) {
				...
				// Set the page in mb_hms_settings
				mbh_update_option( $key . '_page', $page_found );
			}
		...
		if ( $option ) {
			...
			// Set the page in mb_hms_settings
			mbh_update_option( $key . '_page', $page_id );
		}
