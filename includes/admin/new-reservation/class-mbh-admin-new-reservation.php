...
if ( ! class_exists( 'MBH_Admin_New_Reservation' ) ) :

/**
 * MBH_Admin_New_Reservation Class
 */
class MBH_Admin_New_Reservation {
	...
	public static function new_reservation_action() {
		if ( isset( $_POST[ 'mb_hms_admin_add_new_reservation' ] ) ) {

			ob_start();

			try {
				if ( empty( $_POST[ '_wpnonce' ] ) || ! wp_verify_nonce( $_POST[ '_wpnonce' ], 'mb_hms_admin_process_new_reservation' ) ) {
				...
				foreach ( MBH_Meta_Box_Reservation_Data::get_guest_details_fields() as $key => $field ) {
					// Validation: Required fields
					if ( isset( $field[ 'required' ] ) && $field[ 'required' ] && empty( self::$form_data[ $key ] ) ) {
						throw new Exception( sprintf( esc_html__( 'Please fill the required fields.', 'wp-mb_hms' ) ) );
					}
				}

				// Check checkin and checkout dates
				if ( ! MBH_Formatting_Helper::is_valid_checkin_checkout( self::$checkin, self::$checkout ) ) {
	...
	public static function add_to_reservation( $room ) {
		...
		$reservation_item_key = mbh_generate_item_key( $room_id, $rate_id );
	...
	public static function calculate_totals() {
		foreach ( self::$reservation_contents as $reservation_item_key => $values ) {
			...
			if ( ! $line_price ) {
				...
				throw new Exception( esc_html__( 'Sorry, this room cannot be reserved.', 'wp-mb_hms' ) );
			}
		...
		// Calculate taxes
		self::$tax_total = mbh_is_tax_enabled() ? mbh_calculate_tax( self::$reservation_contents_total ) : 0;

		$total           = self::$reservation_contents_total + mbh_calculate_tax( self::$reservation_contents_total );
		self::$total     = $total;
	}
	...
	public static function create_reservation() {
		...
		try {
			...
			if ( is_wp_error( $reservation ) ) {
				...
				$booking_id = mbh_add_booking( $reservation_id, self::$checkin, self::$checkout, 'pending' );
			...
			foreach ( MBH_Meta_Box_Reservation_Data::get_guest_details_fields() as $address_key => $address_value ) {
			...
			$reservation->add_reservation_note( esc_html__( 'Reservation manually created by admin.', 'wp-mb_hms' ) );
...
MBH_Admin_New_Reservation::init();
