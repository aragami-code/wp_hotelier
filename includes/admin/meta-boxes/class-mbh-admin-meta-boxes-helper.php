...
if ( ! class_exists( 'MBH_Meta_Boxes_Helper' ) ) :

/**
 * MBH_Meta_Boxes_Helper Class
 */
class MBH_Meta_Boxes_Helper {
	/**
	...
	public static function text_input( $field ) {
		...
		if ( $data_type == 'price' ) {
			$field[ 'class' ] .= ' mbh-input-price';
			$field[ 'value' ]  = MBH_Formatting_Helper::localized_amount( $field[ 'value' ] );
		}
		...
		if ( ! empty( $field[ 'description' ] ) ) {
			if ( isset( $field[ 'desc_tip' ] ) && false !== $field[ 'desc_tip' ] ) {
				echo '<span title="' . esc_attr( $field[ 'description' ] ) . '" class="hastip"><i class="mbh-icon mbh-help-circled"></i></span>';
			} else {
	...
	public static function price_per_day( $field ) {
		...
		for ( $i = 0; $i < 7; $i ++ ) {
			...
			if ( get_post_meta( $thepostid, $field[ 'id' ], true ) && ( is_array( get_post_meta( $thepostid, $field[ 'id' ], true ) ) ) ) {
				...
				$value = 'value="' .  MBH_Formatting_Helper::localized_amount( $meta[ $day_index ] ) . '"';
			} else {
				$value = '';
			}

			echo '<span class="single-day-field"><label><span>' . $day_initial . '</span><input type="text" name="' . $field[ 'name' ] . '[' . $day_index . ']" ' . $value . ' placeholder="' . MBH_Meta_Box_Room_Settings::get_price_placeholder() . '" class="mbh-input-price" /></label></span>';
		}
