jQuery(function ($) {
	...
	var MBH_Room_Meta = {
		...
		sortable: function () {
			$('.room-conditions').sortable({
				...
				axis: 'y',
				update: function () {
					MBH_Room_Meta.update_conditions_keys($(this));
				}
			});
		},
		...
		add_condition: function () {
			$('.room-advanced-settings').on('click', '.add-condition', function (e) {
				...
				var clone = MBH_Room_Meta.clone_condition(condition);
		...
		remove_condition: function () {
			$('.room-advanced-settings').on('click', '.remove-condition', function (e) {
				...
				MBH_Room_Meta.update_conditions_keys(table);
			});
		},
		...
		show_errors: function () {
			...
			.on('keyup change', 'input.mbh-input-price', function () {
				var value = $(this).val();
	...
	var MBH_Room_Variations = {
		...
		add_variation: function () {
			...
			variation_button.on('click', function (e) {
				...
				var clone = MBH_Room_Variations.clone_variation(variation);

				clone.insertAfter(variation);
				MBH_Room_Meta.sortable();
			});
		},
	...
	var MBH_Room_Additional_Settings = {
		init: function () {
	...
	var MBH_Room_Gallery = {
		...
		sort_images: function () {
			...
			room_images.sortable({
				...
				helper: 'clone',
				opacity: 0.65,
				placeholder: 'mbh-image-sortable-placeholder',
				start: function (event, ui) {
					var styles = {
						backgroundColor: '#f6f6f6'
					};
	...
	$(document).ready(function () {
		MBH_Room_Meta.init();
		MBH_Room_Variations.init();
		MBH_Room_Additional_Settings.init();
		MBH_Room_Gallery.init();
	});
});
