jQuery(function ($) {
	...
	var MBH_Settings = {
		...
		show_uploader: function () {
			var uploader_button = $('.mbh-uploader');
		...
		show_unforce_ssl: function () {
			var force_ssl_input = $('input[name="mb_hms_settings[enforce_ssl_booking]"]');
			var unforce_ssl_field = $('#mb_hms_settings\\[unforce_ssl_booking\\]').closest('tr');
		...
		show_pets_message: function () {
			var pets_input = $('input[name="mb_hms_settings[hotel_pets]"]');
			var pets_message = $('#mb_hms_settings\\[hotel_pets_message\\]').closest('tr');
		...
		show_book_now_quantity: function () {
			var book_now_behaviour = $('input[name="mb_hms_settings[book_now_redirect_to_booking_page]"]');
			var book_now_quantity = $('#mb_hms_settings\\[book_now_allow_quantity_selection\\]').closest('tr');
		...
		add_seasonal_rule: function () {
			$('#mb_hms-seasonal-schema-table').on('click', '.add-rule', function (e) {
				e.preventDefault();

				var rule = $('#mb_hms-seasonal-schema-table').find('tr.rule-row').last();
				var clone = MBH_Settings.clone_rule(rule);

				clone.insertAfter(rule);
				MBH_Settings.datepicker();
			});
		},
		...
		remove_rule: function () {
			$('#mb_hms-seasonal-schema-table').on('click', '.remove-rule', function (e) {
				e.preventDefault();

				var button = $(this);
				var rule = button.parent().parent();
				var rules = $('#mb_hms-seasonal-schema-table').find('tr.rule-row');
				...
				MBH_Settings.update_rule_keys();
			});
		},

		update_rule_keys: function () {
			$('#mb_hms-seasonal-schema-table').find('tr.rule-row').each(function (index) {
				var row = $(this);
				var i = index + 1;
		...
		sort_rules: function () {
			$('#mb_hms-seasonal-schema-table').sortable({
				items: 'tr',
				handle: '.sort-rules',
				opacity: 0.65,
				axis: 'y',
		...
		datepicker: function () {
			var table = $('#mb_hms-seasonal-schema-table');
