...
<div class="room__image room__image--listing">
	...
	<?php
		if ( has_post_thumbnail() ) {
			...
			echo apply_filters( 'mb_hms_room_list_image_html', sprintf( '<a href="%s" data-size="%sx%s" data-index="0" class="room__gallery-thumbnail room__gallery-thumbnail--visible room__gallery-thumbnail--listing" title="%s">%s</a>', $image_link, $image_width, $image_height, $image_caption, $image ), $post->ID );

		} else {

			echo '<a href="' . esc_url ( get_the_permalink() ) . '">' . mbh_placeholder_img( 'room_thumbnail' ) . '</a>';
