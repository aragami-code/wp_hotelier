...
if ( $reservation->has_status( 'pending' ) ) {
	...
	if ( $reservation->needs_payment() ) {
		echo sprintf( esc_html__( 'Hello %s, a reservation has been created for you on %s. To pay for this reservation please use the following link: %s', 'wp-mb_hms' ), $reservation->get_formatted_guest_full_name(), get_bloginfo( 'name', 'display' ), $reservation->get_booking_payment_url() ) . "\n\n";
	} else {
		echo sprintf( esc_html__( 'Hello %s, a reservation has been created for you on %s.', 'wp-mb_hms' ), $reservation->get_formatted_guest_full_name(), get_bloginfo( 'name', 'display' ) ) . "\n\n";
	}
}
